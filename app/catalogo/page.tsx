...
import { type Pelicula } from './CatalogoInteractivo'
...
export default async function CatalogoPage() {
  ...
  const peliculas: Pelicula[] = peliculasRaw.map((p: any) => {
    ...
    return {
      id: p.id, tmdb_id: p.tmdb_id ?? null, titulo: p.titulo_latino || p.titulo, titulo_ingles: p.titulo_ingles ?? null,
      anio: p.anio ?? null, nota_imdb: p.nota_imdb ?? null, rt_score: p.rt_score ?? null, metacritic_score: p.metacritic_score ?? null,
      runtime: p.runtime ?? null, boxoffice: p.boxoffice ?? null, categoria: p.categoria ?? null,
      poster_path: p.poster_path ?? null, backdrop_path: p.backdrop_path ?? null,
      oscars: p.oscars ?? null, imdb_id: p.imdb_id ?? null, youtube_trailer_key: p.youtube_trailer_key ?? null,
      sinopsis: enr.sinopsis_chilensis ?? null, plataformas: platMap[p.id] ?? [],
      es_review_autor: enr.es_review_autor || false, sello_bret: enr.sello_bret || false,
      director: enr.director ?? null, director_oscars: enr.director_oscars ?? null,
      actores: enr.actores ?? null, actores_oscars: enr.actores_oscars ?? null,
      compositor: enr.compositor ?? null, compositor_oscars: enr.compositor_oscars ?? null,
      generos: ((enr.generos as string[]) || []).map(normalizarGenero),
      video_clip_url: enr.video_clip_url ?? null, keywords: ((enr.keywords as string[]) || []),
      tagline: p.tagline ?? null, certification: p.certification ?? null,
    }
  })
  ...
  const series: Pelicula[] = seriesRaw.map((s: any) => {
    ...
    return {
      id: s.id, tmdb_id: s.tmdb_id ?? null, titulo: s.titulo_latino || s.titulo, titulo_ingles: s.titulo_ingles ?? null,
      anio: s.anio_inicio ?? null, nota_imdb: s.nota_imdb ?? null, rt_score: null, metacritic_score: null,
      runtime: s.episode_runtime ?? null, boxoffice: null, categoria: s.categoria ?? null,
      poster_path: s.poster_path ?? null, backdrop_path: s.backdrop_path ?? null,
      oscars: null, imdb_id: s.imdb_id ?? null, youtube_trailer_key: s.youtube_trailer_key ?? null,
      sinopsis: enr.sinopsis_chilensis ?? null, plataformas: seriesPlatMap[s.id] ?? [],
      es_review_autor: false, sello_bret: false, director: enr.director ?? null, director_oscars: null,
      actores: actoresNorm, actores_oscars: null, compositor: enr.compositor ?? null, compositor_oscars: null,
      generos: ((enr.generos as string[]) || []).map(normalizarGenero),
      video_clip_url: null, keywords: ((enr.keywords as string[]) || []),
      tagline: s.tagline ?? null, certification: s.certification ?? null,
      _isSerie: true,
    } as Pelicula
  })
