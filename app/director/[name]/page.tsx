    ...
    <PageShell fullBleed>
          ...
          <div className="mt-4 flex items-end gap-5">
            {person?.profile_path && (
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden bg-zinc-800 shrink-0 ring-4 ring-zinc-950">
                <img loading="lazy" src={`https://image.tmdb.org/t/p/w185${person.profile_path}`} alt={directorName} className="w-full h-full object-cover" />
              </div>
            )}
            <div>
              ...
              <div className="flex items-center gap-3 mt-2 flex-wrap">
                ...
                {bestPicture > 0 && (
                  <Pill variant="gold">
                    <img loading="lazy" src="/oscar.png" alt="Oscar" className="h-4 w-auto" /> {bestPicture} Mejor Película
                  </Pill>
                )}
                {dirOscars != null && dirOscars > 0 && (
                  <Pill variant="gold">{dirOscars} Oscar{dirOscars > 1 ? 's' : ''} personales</Pill>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="flex items-center gap-2 flex-wrap mb-6">
          ...
          {dirOscars != null && dirOscars > 0 && <Pill variant="gold">{dirOscars} Óscar{dirOscars > 1 ? 'es' : ''}</Pill>}
          {bestPicture > 0 && <Pill variant="gold">{bestPicture} Mejor Película</Pill>}
        </div>

        {/* Collaborators — collapsible details */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <details className="bg-zinc-900 rounded-2xl group" open>
              <summary className="p-4 text-xs text-zinc-500 uppercase tracking-wider font-bold cursor-pointer list-none flex items-center justify-between">
                Cast (Actores) <span className="text-zinc-600 group-open:rotate-180 transition-transform">▾</span>
              </summary>
