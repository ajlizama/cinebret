...
export default function MapaPruebaPage() {
  ...
  const graphData = useMemo(() => {
    ...
    // Sort by IMDB desc, take top N, filter by mood
    let sortedNodes = [...rawGraph.nodes].sort((a, b) => (b.imdb || 0) - (a.imdb || 0))
    if (moodFilter.length > 0) {
      sortedNodes = sortedNodes.filter(n => moodFilter.includes(n.categoria || ''))
    }
  ...
  return (
    <main className="fixed inset-0 flex flex-col bg-zinc-950 overflow-hidden">
      ...
      <div ref={containerRef} className="relative flex-1 min-h-0">
        ...
        {/* Mood + Platform filters — bottom, visible when no selection */}
        {!selectedNode && pathNodes.length === 0 && (
          <div className="absolute bottom-3 left-2 right-2 z-10">
            <div className="bg-zinc-900/90 backdrop-blur-sm rounded-xl px-3 py-2 space-y-1.5">
              <div className="flex gap-1.5 overflow-x-auto scrollbar-none">
                {CAT_IDS.map(cat => {
                  ...
                  return (
                    <button
                      ...
                      className={`shrink-0 px-2.5 py-1 rounded-lg text-[10px] font-semibold transition-colors ${active ? 'bg-yellow-400 text-zinc-950' : 'bg-zinc-800 text-zinc-400'}`}
                    >
                      {CAT_LABELS[cat]}
                    </button>
                  )
                ...
                {moodFilter.length > 0 && (
                  <button onClick={() => setMoodFilter([])} className="shrink-0 px-1.5 py-1 text-zinc-500 text-[10px]">✕</button>
                )}
              </div>
              <div className="flex gap-1 overflow-x-auto scrollbar-none">
                {PLATAFORMAS.map(p => (
                  <div key={p.id} className="shrink-0 h-6 w-11 rounded bg-white/90 flex items-center justify-center">
