...
import CatalogoInteractivoPrueba from './CatalogoInteractivoPrueba'
import { type Pelicula } from '../catalogo/CatalogoInteractivo'
...
const GENEROS_NORMALIZE: Record<string, string> = {
  ...
  'Sports': 'Deporte', 'Accion': 'Acción', 'Animacion': 'Animación', 'Biografia': 'Biografía',
  'Biográfico': 'Biografía', 'Fantasia': 'Fantasía', 'Familiar': 'Familia',
  'Ciencia Ficción': 'Ciencia ficción', 'Ciencia Ficcion': 'Ciencia ficción',
  'Musica': 'Música', 'Deportes': 'Deporte', 'Unknown': 'Otros', 'Desconocido': 'Otros',
}
...
export default async function PruebaPage() {
  // Same data fetching as catalogo/page.tsx
  ...
  const [watchProvidersRaw, catalogosRaw, peliculasRaw] = await Promise.all([
    fetchAllPages((from, to) =>
      supabase.from('watch_providers').select('pelicula_id, platform_key')
        .eq('provider_type', 'flatrate').not('platform_key', 'is', null).range(from, to)
    ),
    fetchAllPages((from, to) =>
  ...
  const peliculas: Pelicula[] = peliculasRaw.map((p: any) => {
    ...
    return {
      id: p.id, tmdb_id: p.tmdb_id ?? null, titulo: p.titulo, titulo_ingles: p.titulo_ingles ?? null,
      anio: p.anio ?? null, nota_imdb: p.nota_imdb ?? null, rt_score: p.rt_score ?? null,
      metacritic_score: p.metacritic_score ?? null, runtime: p.runtime ?? null, boxoffice: p.boxoffice ?? null,
      categoria: p.categoria ?? null, poster_path: p.poster_path ?? null, backdrop_path: p.backdrop_path ?? null,
  ...
  return (
    <main className="min-h-screen bg-zinc-950">
      <Nav active="inicio" />
      <CatalogoInteractivoPrueba peliculas={peliculas} trendingIds={trendingIds} />
    </main>
  )
}
